...
const ItemDetails = ({
  ...
  item: CartMenuItem | CartPackageItem;
  type: "menu" | "package";
}) => (
      ...
      <>
        {(item as CartMenuItem).variants.length > 0 && (
          <div className="text-xs text-gray-600">
            ...
            {(item as CartMenuItem).variants.map((v, i) => (
        ...
        {(item as CartMenuItem).addons.length > 0 && (
          <div className="text-xs text-gray-600">
            ...
            {(item as CartMenuItem).addons.map((a, i) => (
              <div key={i} className="ml-2">
...
const ItemCard = ({
  ...
  const hasDetails =
    type === "menu"
      ? (item as CartMenuItem).variants.length > 0 ||
        (item as CartMenuItem).addons.length > 0
      : (item as CartPackageItem).menus.some(
          (m) => m.variants.length > 0 || m.addons.length > 0
        );
      ...
      <div className="flex flex-col md:flex-row">
        <div className="relative md:w-48 w-full h-48 md:h-auto flex-shrink-0 bg-gray-100">
          <Image
            src={getFullImageUrl(
              type === "menu"
                ? (item as CartMenuItem).image_url
                : (item as CartPackageItem).package_image
            )}
            alt={
              type === "menu"
                ? (item as CartMenuItem).menu_name
                : (item as CartPackageItem).package_name
            }
            fill
        ...
        <div className="flex-1 p-4 flex flex-col justify-between">
          <div>
            <div className="flex items-baseline gap-2 mb-2">
              ...
              {item.category_name && (
                <Badge
                  variant="outline"
                  className="border-orange-300 text-orange-600 bg-orange-50 text-xs"
                >
                  {item.category_name}
                </Badge>
              )}
            ...
            <p className="text-sm text-gray-600 mb-3 line-clamp-2">
              {type === "menu"
                ? (item as CartMenuItem).menu_description
                : (item as CartPackageItem).package_description}
            </p>

            {(hasDetails || type === "package") && (
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="text-xl font-bold text-orange-500">
              RM
              {(type === "menu"
                ? calculateItemTotal(item as CartMenuItem)
                : calculatePackageTotal(item as CartPackageItem)
              ).toFixed(2)}
            </div>
...
const OrderConfirmation: React.FC<Step4Props> = ({
  ...
  const isDineIn = selectedService?.display_name
    .toLowerCase()
    .includes("dine in");
          ...
          <CardContent className="p-6 text-sm space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium capitalize">
                {selectedService?.display_name || serviceType}
              </span>
            </div>
              ...
              <>
                ...
                {pax && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{pax} people</span>
                  </div>
                )}
                {reservationDate && (
                  <div className="flex justify-between">
