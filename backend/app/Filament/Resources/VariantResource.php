...
// 定义VariantResource类，它代表后台管理中的“规格”资源
class VariantResource extends Resource
{
    // 指定此资源关联的Eloquent模型
    protected static ?string $model = Variant::class;

    // 指定在导航菜单中显示的图标（Heroicon图标库的 "adjustments-vertical" 图标）
        ...
        return $table
            ...
            ->columns([
                ...
                ToggleColumn::make('variant_status')
                    ...
                    ->action(fn(Variant $record) => $record->update(['variant_status' => !$record->variant_status]))
                    // [优化] 增加鼠标悬停提示，提升用户体验
                    ->tooltip(fn(Variant $record): string => $record->variant_status ? '当前为启用状态，点击禁用' : '当前为禁用状态，点击启用'),
